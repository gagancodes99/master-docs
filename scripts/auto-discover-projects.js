 ...
 * Optionally runs multi-folder discovery, then scans docs/projects/ directory 
 * and automatically updates sidebar and dashboard
...
// Check if multi-folder discovery should run first
const configPath = path.join(__dirname, '../project-discovery.config.json');
const runFolderDiscovery = process.env.RUN_FOLDER_DISCOVERY !== 'false' && fs.existsSync(configPath);

if (runFolderDiscovery) {
  try {
    const { discoverProjects } = require('./discover-projects-from-folders');
    console.log('🔄 Running multi-folder discovery first...\n');
    discoverProjects();
    console.log('\n');
  } catch (error) {
    console.warn('⚠️  Multi-folder discovery failed, continuing with existing docs:', error.message);
  }
}
 ...
 * This checks both the normalized version and the original filename
 */
function getDocusaurusDocId(filePath) {
  const filename = path.basename(filePath);
  const normalized = normalizeDocId(filename);
  
  // Try to read the file to see if Docusaurus has any special handling
  // For now, we'll use the normalized version, but fall back to original if needed
  return normalized;
}
